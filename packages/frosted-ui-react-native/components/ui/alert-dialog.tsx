    ...
    <AlertDialogPortal hostName={portalHost}>
      <AlertDialogOverlay>
        <AlertDialogPrimitive.Content
          className={cn(
            'border-stroke z-50 flex w-full max-w-[calc(100%-2rem)] flex-col gap-4 rounded-lg border bg-background p-6 shadow-lg shadow-black/5 sm:max-w-lg',
            Platform.select({
...
function AlertDialogTitle({
  ...
  return (
    <AlertDialogPrimitive.Title
      className={cn('text-gray-12 text-lg font-semibold', className)}
      {...props}
    />
  );
}
...
function AlertDialogDescription({
  ...
  return (
    <AlertDialogPrimitive.Description
      className={cn('text-gray-a10 text-sm', className)}
      {...props}
    />
  );
}
