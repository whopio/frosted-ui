import * as React from 'react';
import { IconProps } from './types';

export const HomeBold20 = ({ color = 'currentColor', ...props }: IconProps) => {
  return (
    <svg
      width="20"
      height="20"
      viewBox="0 0 20 20"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      data-fui-icon="true"
      {...props}
    >
      <path
        d="M2 9.30746C2 8.41281 2.38128 7.56057 3.04827 6.96433L7.5341 2.95433C8.93839 1.699 11.0616 1.699 12.4659 2.95433L16.9517 6.96433C17.6187 7.56057 18 8.41281 18 9.30746V15.6428C18 16.9446 16.9447 18 15.6429 18H13.5414C12.6736 18 11.97 17.2964 11.97 16.4285V12.97C11.97 11.882 11.088 11 10 11C8.912 11 8.03 11.882 8.03 12.97V16.4285C8.03 17.2964 7.32645 18 6.45857 18H4.35714C3.05533 18 2 16.9446 2 15.6428V9.30746Z"
        stroke={color}
        strokeWidth="2"
      />
    </svg>
  );
};

HomeBold20.category = 'Buildings';

export default HomeBold20;
