import * as React from 'react';
import { IconProps } from './types';

export const ReceiptBoldFilled20 = ({ color = 'currentColor', ...props }: IconProps) => {
  return (
    <svg
      width="20"
      height="20"
      viewBox="0 0 20 20"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      data-fui-icon="true"
      {...props}
    >
      <path
        d="M14.75 0C16.5449 0 18 1.45507 18 3.25V18.0869C17.9997 19.2547 16.7568 20.0024 15.7246 19.4561L13.293 18.1689L10.7988 19.4971C10.299 19.7629 9.69966 19.7626 9.2002 19.4961L6.71289 18.1689L4.27344 19.458C3.24136 20.0028 2.00026 19.2549 2 18.0879V3.25C2 1.45507 3.45507 0 5.25 0H14.75ZM7.00098 9C6.44869 9 6.00098 9.44771 6.00098 10C6.00098 10.5523 6.44869 11 7.00098 11H10.001C10.5533 11 11.001 10.5523 11.001 10C11.001 9.44771 10.5533 9 10.001 9H7.00098ZM7.00098 5C6.44869 5 6.00098 5.44772 6.00098 6C6.00098 6.55228 6.44869 7 7.00098 7H13.001C13.5533 7 14.001 6.55228 14.001 6C14.001 5.44772 13.5533 5 13.001 5H7.00098Z"
        fill={color}
      />
    </svg>
  );
};

ReceiptBoldFilled20.category = 'Money & Shopping';

export default ReceiptBoldFilled20;
