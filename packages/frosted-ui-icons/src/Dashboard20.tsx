import * as React from 'react';
import { IconProps } from './types';

export const Dashboard20 = ({ color = 'currentColor', ...props }: IconProps) => {
  return (
    <svg
      width="20"
      height="20"
      viewBox="0 0 20 20"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      data-fui-icon="Dashboard20"
      {...props}
    >
      <path
        d="M13.65 1C16.605 1 19 3.395 19 6.35v7.3c0 2.955-2.395 5.35-5.35 5.35h-7.3C3.395 19 1 16.605 1 13.65v-7.3C1 3.395 3.395 1 6.35 1h7.3zm-7.3 1.5C4.224 2.5 2.5 4.224 2.5 6.35v7.3c0 2.126 1.724 3.85 3.85 3.85h7.3c2.126 0 3.85-1.724 3.85-3.85v-7.3c0-2.126-1.724-3.85-3.85-3.85h-7.3zm7.377 5.087c.298-.289.773-.282 1.061.016.288.297.282.772-.015 1.06l-3.216 3.122c-.437.424-1.131.424-1.568 0l-1.535-1.49-2.182 2.118c-.297.288-.772.282-1.06-.015-.289-.298-.282-.773.016-1.061L7.67 8.965c.409-.397 1.045-.422 1.482-.074l.085.074 1.535 1.49 2.954-2.868z"
        fill={color}
      />
    </svg>
  );
};

Dashboard20.category = 'Stats & Charts';

export default Dashboard20;
