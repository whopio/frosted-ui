import * as React from 'react';
import { IconProps } from './types';

export const ProfileCircle12 = ({ color = 'currentColor', ...props }: IconProps) => {
  return (
    <svg
      width="12"
      height="12"
      viewBox="0 0 12 12"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      data-fui-icon="true"
      {...props}
    >
      <path
        d="M6 0C9.31371 0 12 2.68629 12 6C12 9.31371 9.31371 12 6 12C2.68629 12 0 9.31371 0 6C0 2.68629 2.68629 0 6 0ZM6.00098 9C5.15806 9 4.40266 9.37951 3.89746 9.97852C4.5251 10.3109 5.24033 10.5 6 10.5C6.75936 10.5 7.47411 10.3106 8.10156 9.97852C7.59668 9.38002 6.84328 9.00029 6.00098 9ZM6 1.5C3.51472 1.5 1.5 3.51472 1.5 6C1.5 7.18475 1.95863 8.26183 2.70703 9.06543C3.48583 8.11159 4.67162 7.5 6.00098 7.5C7.32971 7.50029 8.51348 8.11211 9.29199 9.06543C10.0407 8.26178 10.5 7.18499 10.5 6C10.5 3.51472 8.48528 1.5 6 1.5ZM6 2.25C7.24264 2.25 8.25 3.25736 8.25 4.5C8.25 5.74264 7.24264 6.75 6 6.75C4.75736 6.75 3.75 5.74264 3.75 4.5C3.75 3.25736 4.75736 2.25 6 2.25ZM6 3.75C5.58579 3.75 5.25 4.08579 5.25 4.5C5.25 4.91421 5.58579 5.25 6 5.25C6.41421 5.25 6.75 4.91421 6.75 4.5C6.75 4.08579 6.41421 3.75 6 3.75Z"
        fill={color}
      />
    </svg>
  );
};

ProfileCircle12.category = 'People';

export default ProfileCircle12;
