import * as React from 'react';
import { IconProps } from './types';

export const CompassFilled16 = ({ color = 'currentColor', ...props }: IconProps) => {
  return (
    <svg
      width="16"
      height="16"
      viewBox="0 0 16 16"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      data-fui-icon="true"
      {...props}
    >
      <g clipPath="url(#clip0_6096_8)">
        <path
          d="M8 0.583008C12.0958 0.583184 15.4158 3.90416 15.416 8C15.4158 12.0959 12.0959 15.4158 8 15.416C3.90416 15.4158 0.583184 12.0958 0.583008 8C0.583206 3.90418 3.90418 0.583206 8 0.583008ZM10.7031 6.3291C10.9477 5.68424 10.3158 5.05227 9.6709 5.29688L7.3418 6.18066C6.80664 6.38365 6.38365 6.80664 6.18066 7.3418L5.29688 9.6709C5.05227 10.3158 5.68424 10.9477 6.3291 10.7031L8.6582 9.81934C9.19336 9.61635 9.61635 9.19336 9.81934 8.6582L10.7031 6.3291Z"
          fill={color}
        />
      </g>
      <defs>
        <clipPath id="clip0_6096_8">
          <path fill={color} d="M0 0H16V16H0z" />
        </clipPath>
      </defs>
    </svg>
  );
};

export default CompassFilled16;
