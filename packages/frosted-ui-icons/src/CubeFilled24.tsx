import * as React from 'react';
import { IconProps } from './types';

export const CubeFilled24 = ({ color = 'currentColor', ...props }: IconProps) => {
  return (
    <svg
      width="24"
      height="24"
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      data-fui-icon="CubeFilled24"
      {...props}
    >
      <path
        d="M9.5 1.443c1.547-.893 3.453-.893 5 0l5.393 3.114c1.546.893 2.5 2.543 2.5 4.33v6.226c0 1.787-.954 3.437-2.5 4.33L14.5 22.557c-1.547.893-3.453.893-5 0l-5.393-3.114c-1.546-.893-2.5-2.544-2.5-4.33V8.887c0-1.787.954-3.437 2.5-4.33L9.5 1.443zm9.276 6.685c-.205-.36-.663-.485-1.023-.28L12 11.137 6.247 7.849c-.36-.206-.818-.08-1.023.279-.206.36-.08.818.279 1.023l5.747 3.284V19c0 .414.336.75.75.75s.75-.336.75-.75v-6.565l5.747-3.284c.36-.205.485-.663.28-1.023z"
        fill={color}
      />
    </svg>
  );
};

CubeFilled24.category = 'Objects';

export default CubeFilled24;
