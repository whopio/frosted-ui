import * as React from 'react';
import { IconProps } from './types';

export const CouponActiveBold12 = ({ color = 'currentColor', ...props }: IconProps) => {
  return (
    <svg
      width="12"
      height="12"
      viewBox="0 0 12 12"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      data-fui-icon="CouponActiveBold12"
      {...props}
    >
      <path
        d="M4.099 0c.552 0 .964.314 1.192.668l.086.153.052.091c.133.2.346.316.571.317.257 0 .499-.152.623-.408L6.71.668C6.937.314 7.349 0 7.901 0H9c1.105 0 2 .895 2 2v8c0 1.105-.895 2-2 2H7.901c-.631 0-1.079-.411-1.278-.821-.124-.256-.366-.408-.623-.408s-.499.152-.623.408c-.2.41-.647.82-1.278.821H3c-1.105 0-2-.895-2-2V2c0-1.105.895-2 2-2h1.099zM3 10h.752C4.23 9.269 5.051 8.772 6 8.771c.95 0 1.77.498 2.248 1.229H9V2h-.752C7.769 2.731 6.949 3.229 6 3.229c-.95 0-1.77-.498-2.248-1.229H3v8zm3.793-6.207c.39-.39 1.024-.39 1.414 0 .39.39.39 1.024 0 1.414l-3 3c-.39.39-1.024.39-1.414 0-.39-.39-.39-1.024 0-1.414l3-3zM7.5 6.5c.552 0 1 .448 1 1s-.448 1-1 1-1-.448-1-1 .448-1 1-1zm-3-3c.552 0 1 .448 1 1s-.448 1-1 1-1-.448-1-1 .448-1 1-1z"
        fill={color}
      />
    </svg>
  );
};

CouponActiveBold12.category = 'Money & Shopping';

export default CouponActiveBold12;
