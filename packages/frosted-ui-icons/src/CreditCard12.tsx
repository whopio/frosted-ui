import * as React from 'react';
import { IconProps } from './types';

export const CreditCard12 = ({ color = 'currentColor', ...props }: IconProps) => {
  return (
    <svg
      width="12"
      height="12"
      viewBox="0 0 12 12"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      data-fui-icon="CreditCard12"
      {...props}
    >
      <path
        d="M9.25 1.5C10.769 1.5 12 2.731 12 4.25v3.5c0 1.519-1.231 2.75-2.75 2.75h-6.5C1.231 10.5 0 9.269 0 7.75v-3.5C0 2.731 1.231 1.5 2.75 1.5h6.5zM1.5 7.75C1.5 8.44 2.06 9 2.75 9h6.5c.69 0 1.25-.56 1.25-1.25V5.5h-9v2.25zM4.75 6.5c.414 0 .75.336.75.75S5.164 8 4.75 8h-1.5c-.414 0-.75-.336-.75-.75s.336-.75.75-.75h1.5zm-2-3.5c-.605 0-1.109.43-1.225 1h8.95c-.116-.57-.62-1-1.225-1h-6.5z"
        fill={color}
      />
    </svg>
  );
};

CreditCard12.category = 'Money & Shopping';

export default CreditCard12;
