import * as React from 'react';
import { IconProps } from './types';

export const LockOpen24 = ({ color = 'currentColor', ...props }: IconProps) => {
  return (
    <svg
      width="24"
      height="24"
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      data-fui-icon="true"
      {...props}
    >
      <path
        d="M12 0C15.5802 0 18.5314 2.68751 18.9492 6.15527C18.9985 6.56621 18.7058 6.93954 18.2949 6.98926C17.8837 7.03881 17.5095 6.74522 17.46 6.33398C17.1317 3.61084 14.8119 1.5 12 1.5C8.96243 1.5 6.5 3.96243 6.5 7V10H18.25C20.3211 10 22 11.6789 22 13.75V20.25C22 22.3211 20.3211 24 18.25 24H5.75C3.67893 24 2 22.3211 2 20.25V13.75C2 11.9357 3.28824 10.4216 5 10.0742V7C5 3.13401 8.13401 0 12 0ZM5.75 11.5C4.50736 11.5 3.5 12.5074 3.5 13.75V20.25C3.5 21.4926 4.50736 22.5 5.75 22.5H18.25C19.4926 22.5 20.5 21.4926 20.5 20.25V13.75C20.5 12.5074 19.4926 11.5 18.25 11.5H5.75Z"
        fill={color}
      />
    </svg>
  );
};

export default LockOpen24;
