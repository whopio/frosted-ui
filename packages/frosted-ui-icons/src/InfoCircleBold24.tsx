import * as React from 'react';
import { IconProps } from './types';

export const InfoCircleBold24 = ({ color = 'currentColor', ...props }: IconProps) => {
  return (
    <svg
      width="24"
      height="24"
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      data-fui-icon="true"
      {...props}
    >
      <path
        d="M12 0.5C18.3513 0.5 23.5 5.64873 23.5 12C23.5 18.3513 18.3513 23.5 12 23.5C5.64873 23.5 0.5 18.3513 0.5 12C0.5 5.64873 5.64873 0.5 12 0.5ZM12 2.5C6.75329 2.5 2.5 6.75329 2.5 12C2.5 17.2467 6.75329 21.5 12 21.5C17.2467 21.5 21.5 17.2467 21.5 12C21.5 6.75329 17.2467 2.5 12 2.5ZM12.001 10.25C12.5529 10.2503 13.0008 10.698 13.001 11.25V17C13.0008 17.552 12.5529 17.9997 12.001 18C11.4488 18 11.0012 17.5521 11.001 17V11.25C11.0012 10.6979 11.4488 10.25 12.001 10.25ZM12.001 6C12.6081 6.00026 13.1004 6.49244 13.1006 7.09961C13.1006 7.70696 12.6083 8.19895 12.001 8.19922C11.3935 8.19922 10.9004 7.70712 10.9004 7.09961C10.9006 6.49228 11.3936 6 12.001 6Z"
        fill={color}
      />
    </svg>
  );
};

InfoCircleBold24.category = 'Interface General';

export default InfoCircleBold24;
