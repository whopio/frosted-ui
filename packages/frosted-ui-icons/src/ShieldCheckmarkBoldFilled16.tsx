import * as React from 'react';
import { IconProps } from './types';

export const ShieldCheckmarkBoldFilled16 = ({ color = 'currentColor', ...props }: IconProps) => {
  return (
    <svg
      width="16"
      height="16"
      viewBox="0 0 16 16"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      data-fui-icon="ShieldCheckmarkBoldFilled16"
      {...props}
    >
      <path
        d="M7.30566 1.14155C7.79828 0.952744 8.34403 0.955222 8.83496 1.14839L12.665 2.65718C13.4723 2.97505 14.0038 3.75446 14.0039 4.62202V8.84077C14.0039 10.7156 12.6417 12.1764 11.4834 13.1044C10.2737 14.0735 8.98191 14.7038 8.59375 14.8837C8.26222 15.0374 7.88378 15.0407 7.54883 14.8896C7.15564 14.7121 5.83248 14.0835 4.5918 13.1162C3.40769 12.1929 2 10.7294 2 8.84077V4.62886C2.00014 3.75457 2.53917 2.97035 3.35547 2.65718L7.30566 1.14155ZM10.707 5.70796C10.3165 5.31745 9.68349 5.31744 9.29297 5.70796L7.08203 7.9189L6.70703 7.5439C6.31653 7.15348 5.68348 7.15348 5.29297 7.5439C4.9027 7.93441 4.9026 8.5675 5.29297 8.95796L6.375 10.04C6.56251 10.2274 6.81694 10.333 7.08203 10.333C7.34709 10.3329 7.60158 10.2273 7.78906 10.04L10.707 7.12202C11.0975 6.73158 11.0973 6.0985 10.707 5.70796Z"
        fill={color}
        fillRule="evenodd"
        clipRule="evenodd"
      />
    </svg>
  );
};

ShieldCheckmarkBoldFilled16.category = 'Security';

export default ShieldCheckmarkBoldFilled16;
