import * as React from 'react';
import { IconProps } from './types';

export const CalendarFilled16 = ({ color = 'currentColor', ...props }: IconProps) => {
  return (
    <svg
      width="16"
      height="16"
      viewBox="0 0 16 16"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      data-fui-icon="CalendarFilled16"
      {...props}
    >
      <path
        d="M11.3799 0.25C11.7941 0.25 12.1299 0.585786 12.1299 1V2.33203C13.8711 2.44986 15.25 3.89618 15.25 5.66699V12.4072C15.25 14.2552 13.7483 15.7498 11.9004 15.75H4.09961C2.25165 15.7498 0.75 14.2552 0.75 12.4072V5.66699C0.75 3.89618 2.12893 2.44986 3.87012 2.33203V1C3.87012 0.585786 4.2059 0.25 4.62012 0.25C5.03428 0.250063 5.37012 0.585826 5.37012 1V2.32422H10.6299V1C10.6299 0.585825 10.9657 0.250063 11.3799 0.25ZM2.25 12.4072C2.25 13.4227 3.07604 14.2498 4.09961 14.25H11.9004C12.924 14.2498 13.75 13.4227 13.75 12.4072V7.97266H2.25V12.4072Z"
        fill={color}
      />
    </svg>
  );
};

CalendarFilled16.category = 'Interface General';

export default CalendarFilled16;
