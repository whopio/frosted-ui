import * as React from 'react';
import { IconProps } from './types';

export const ControlsVertical20 = ({ color = 'currentColor', ...props }: IconProps) => {
  return (
    <svg
      width="20"
      height="20"
      viewBox="0 0 20 20"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      data-fui-icon="ControlsVertical20"
      {...props}
    >
      <path
        d="M8.5 6.75c0 1.815-1.288 3.328-3 3.675v8.825c0 .414-.336.75-.75.75S4 19.664 4 19.25v-8.825c-1.712-.347-3-1.86-3-3.675 0-1.814 1.288-3.327 3-3.674V.75c0-.413.336-.75.75-.75s.75.337.75.75v2.326c1.712.347 3 1.86 3 3.674zm-1.5 0C7 5.508 5.993 4.5 4.75 4.5S2.5 5.508 2.5 6.75C2.5 7.993 3.507 9 4.75 9S7 7.993 7 6.75zm12 6.5c0 1.815-1.288 3.328-3 3.675v2.325c0 .413-.336.75-.75.75s-.75-.337-.75-.75v-2.325c-1.712-.347-3-1.86-3-3.675 0-1.814 1.288-3.327 3-3.674V.75c0-.414.336-.75.75-.75s.75.336.75.75v8.826c1.712.347 3 1.86 3 3.675zm-1.5 0c0-1.242-1.007-2.25-2.25-2.25S13 12.009 13 13.25c0 1.243 1.007 2.25 2.25 2.25s2.25-1.007 2.25-2.25z"
        fill={color}
      />
    </svg>
  );
};

ControlsVertical20.category = 'Interface General';

export default ControlsVertical20;
