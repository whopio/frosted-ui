import * as React from 'react';
import { IconProps } from './types';

export const Home24 = ({ color = 'currentColor', ...props }: IconProps) => {
  return (
    <svg
      width="24"
      height="24"
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      data-fui-icon="true"
      {...props}
    >
      <path
        d="M1.75 11.2233C1.75 10.1255 2.21215 9.07833 3.02324 8.33846L8.94711 2.93473C10.6766 1.35707 13.3234 1.35707 15.0529 2.93474L20.9768 8.33846C21.7878 9.07833 22.25 10.1255 22.25 11.2233V19.3213C22.25 20.9387 20.9388 22.2499 19.3214 22.2499H16.7024C15.6241 22.2499 14.75 21.3758 14.75 20.2975V15.9999C14.75 14.4811 13.5188 13.2499 12 13.2499C10.4812 13.2499 9.25 14.4811 9.25 15.9999V20.2975C9.25 21.3758 8.37589 22.2499 7.29762 22.2499H4.67857C3.06117 22.2499 1.75 20.9387 1.75 19.3213V11.2233Z"
        stroke={color}
        strokeWidth="1.5"
      />
    </svg>
  );
};

Home24.category = 'Buildings';

export default Home24;
