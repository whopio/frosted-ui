import * as React from 'react';
import { IconProps } from './types';

export const Export16 = ({ color = 'currentColor', ...props }: IconProps) => {
  return (
    <svg
      width="16"
      height="16"
      viewBox="0 0 16 16"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      data-fui-icon="true"
      {...props}
    >
      <path
        d="M6.5 2C6.91421 2 7.25 2.33579 7.25 2.75C7.25 3.16421 6.91421 3.5 6.5 3.5H3.25C2.2835 3.5 1.5 4.2835 1.5 5.25V12.75C1.5 13.7165 2.2835 14.5 3.25 14.5H10.75C11.7165 14.5 12.5 13.7165 12.5 12.75V9.5C12.5 9.08579 12.8358 8.75 13.25 8.75C13.6642 8.75 14 9.08579 14 9.5V12.75C14 14.5449 12.5449 16 10.75 16H3.25C1.45507 16 0 14.5449 0 12.75V5.25C0 3.45507 1.45507 2 3.25 2H6.5ZM15.25 0C15.6642 3.6076e-05 16 0.335809 16 0.75V5C16 5.41419 15.6642 5.74996 15.25 5.75C14.8358 5.75 14.5 5.41421 14.5 5V2.56055L8.28027 8.78027C7.98738 9.07317 7.51262 9.07316 7.21973 8.78027C6.92684 8.48738 6.92684 8.01262 7.21973 7.71973L13.4395 1.5H11C10.5858 1.5 10.25 1.16421 10.25 0.75C10.25 0.335786 10.5858 0 11 0H15.25Z"
        fill={color}
      />
    </svg>
  );
};

Export16.category = 'Arrows';

export default Export16;
