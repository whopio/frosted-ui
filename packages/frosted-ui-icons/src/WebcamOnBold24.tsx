import * as React from 'react';
import { IconProps } from './types';

export const WebcamOnBold24 = ({ color = 'currentColor', ...props }: IconProps) => {
  return (
    <svg
      width="24"
      height="24"
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      data-fui-icon="WebcamOnBold24"
      {...props}
    >
      <path
        d="M12.25 3.25c2.72 0 4.999 1.89 5.596 4.429l1.764-1.7c1.27-1.223 3.388-.323 3.388 1.44v9.17c0 1.764-2.117 2.665-3.388 1.441l-1.766-1.702c-.6 2.535-2.876 4.422-5.594 4.422h-5.5C3.574 20.75 1 18.176 1 15V9c0-3.176 2.574-5.75 5.75-5.75h5.5zm-5.5 2C4.679 5.25 3 6.929 3 9v6c0 2.071 1.679 3.75 3.75 3.75h5.5c2.071 0 3.75-1.679 3.75-3.75V9c0-2.071-1.679-3.75-3.75-3.75h-5.5zm11.444 4.872c-.06.058-.125.105-.194.145v3.474c.069.04.134.089.194.147l2.804 2.702V7.42l-2.804 2.702z"
        fill={color}
      />
    </svg>
  );
};

WebcamOnBold24.category = 'Interface General';

export default WebcamOnBold24;
