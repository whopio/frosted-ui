import * as React from 'react';
import { IconProps } from './types';

export const ReceiptFilled20 = ({ color = 'currentColor', ...props }: IconProps) => {
  return (
    <svg
      width="20"
      height="20"
      viewBox="0 0 20 20"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      data-fui-icon="ReceiptFilled20"
      {...props}
    >
      <path
        d="M15.001 0c1.657 0 3 1.343 3 3v15.633c0 .981-1.046 1.61-1.912 1.147l-2.679-1.43-2.723 1.462c-.43.23-.945.231-1.374 0l-2.715-1.461-2.687 1.43C3.045 20.242 2 19.615 2 18.634V3c0-1.657 1.343-3 3-3h10.001zM6.266 9c-.415 0-.75.336-.75.75s.335.75.75.75h3.955c.414 0 .75-.336.75-.75S10.635 9 10.22 9H6.266zm0-4c-.415 0-.75.336-.75.75s.335.75.75.75h7.47c.414 0 .75-.336.75-.75S14.15 5 13.736 5h-7.47z"
        fill={color}
      />
    </svg>
  );
};

ReceiptFilled20.category = 'Money & Shopping';

export default ReceiptFilled20;
