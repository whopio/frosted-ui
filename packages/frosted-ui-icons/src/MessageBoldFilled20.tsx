import * as React from 'react';
import { IconProps } from './types';

export const MessageBoldFilled20 = ({ color = 'currentColor', ...props }: IconProps) => {
  return (
    <svg
      width="20"
      height="20"
      viewBox="0 0 20 20"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      data-fui-icon="true"
      {...props}
    >
      <path
        d="M10 0.5C15.2464 0.500277 19.5 4.7534 19.5 10C19.5 15.2466 15.2464 19.4997 10 19.5C8.65646 19.5 7.37739 19.2165 6.2168 18.7119C5.29623 18.9158 4.35991 19.1493 3.54883 19.3613C1.79829 19.8189 0.180107 18.2006 0.637695 16.4502C0.849756 15.6395 1.08216 14.7014 1.28613 13.7793C0.782474 12.6194 0.5 11.3419 0.5 10C0.5 4.75329 4.75329 0.5 10 0.5ZM6 8.5C5.17157 8.5 4.5 9.17157 4.5 10C4.5 10.8284 5.17157 11.5 6 11.5C6.82843 11.5 7.5 10.8284 7.5 10C7.5 9.17157 6.82843 8.5 6 8.5ZM10 8.5C9.17169 8.50013 8.5 9.17165 8.5 10C8.5 10.8283 9.17169 11.4999 10 11.5C10.8284 11.5 11.5 10.8284 11.5 10C11.5 9.17157 10.8284 8.5 10 8.5ZM14 8.5C13.1716 8.5 12.5 9.17157 12.5 10C12.5 10.8284 13.1716 11.5 14 11.5C14.8284 11.5 15.5 10.8284 15.5 10C15.5 9.17157 14.8284 8.5 14 8.5Z"
        fill={color}
        fillRule="evenodd"
        clipRule="evenodd"
      />
    </svg>
  );
};

MessageBoldFilled20.category = 'Communication';

export default MessageBoldFilled20;
