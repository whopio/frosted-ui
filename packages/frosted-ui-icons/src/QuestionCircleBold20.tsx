import * as React from 'react';
import { IconProps } from './types';

export const QuestionCircleBold20 = ({ color = 'currentColor', ...props }: IconProps) => {
  return (
    <svg
      width="20"
      height="20"
      viewBox="0 0 20 20"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      data-fui-icon="QuestionCircleBold20"
      {...props}
    >
      <path
        d="M10 0.5C15.2467 0.5 19.5 4.75329 19.5 10C19.5 15.2467 15.2467 19.5 10 19.5C4.75329 19.5 0.5 15.2467 0.5 10C0.5 4.75329 4.75329 0.5 10 0.5ZM10 2.5C5.85786 2.5 2.5 5.85786 2.5 10C2.5 14.1421 5.85786 17.5 10 17.5C14.1421 17.5 17.5 14.1421 17.5 10C17.5 5.85786 14.1421 2.5 10 2.5ZM10 13C10.6904 13 11.25 13.5596 11.25 14.25C11.25 14.9404 10.6904 15.5 10 15.5C9.30964 15.5 8.75 14.9404 8.75 14.25C8.75 13.5596 9.30964 13 10 13ZM10 4.75C12.1397 4.75016 13.4998 6.53928 13.5 8.34961C13.5 9.92236 12.4547 11.0538 10.9883 11.3906C10.9198 11.8764 10.5036 12.2502 9.99902 12.25C9.44701 12.2496 9 11.802 9 11.25C9.00039 10.2385 9.80452 9.60612 10.5273 9.44531C11.201 9.29555 11.5 8.88814 11.5 8.34961C11.4998 7.46043 10.8603 6.75016 10 6.75C9.13078 6.75 8.50018 7.431 8.5 8.34961C8.5 8.90189 8.05228 9.34961 7.5 9.34961C6.94772 9.34961 6.5 8.90189 6.5 8.34961C6.50019 6.49055 7.86892 4.75 10 4.75Z"
        fill={color}
      />
    </svg>
  );
};

QuestionCircleBold20.category = 'Interface General';

export default QuestionCircleBold20;
