import * as React from 'react';
import { IconProps } from './types';

export const SubtractFilled24 = ({ color = 'currentColor', ...props }: IconProps) => {
  return (
    <svg
      width="24"
      height="24"
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      data-fui-icon="SubtractFilled24"
      {...props}
    >
      <path
        d="M21.75 10.75C22.4404 10.75 23 11.3096 23 12C23 12.6904 22.4404 13.25 21.75 13.25H2.25C1.55964 13.25 1 12.6904 1 12C1 11.3096 1.55964 10.75 2.25 10.75H21.75Z"
        fill={color}
      />
    </svg>
  );
};

SubtractFilled24.category = 'Interface General';

export default SubtractFilled24;
