import * as React from 'react';
import { IconProps } from './types';

export const ThreeDotsHorizontal24 = ({ color = 'currentColor', ...props }: IconProps) => {
  return (
    <svg
      width="24"
      height="24"
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      data-fui-icon="true"
      {...props}
    >
      <path
        d="M4 10.25C4.9665 10.25 5.75 11.0335 5.75 12C5.75 12.9665 4.9665 13.75 4 13.75C3.0335 13.75 2.25 12.9665 2.25 12C2.25 11.0335 3.0335 10.25 4 10.25ZM12 10.25C12.9665 10.25 13.75 11.0335 13.75 12C13.75 12.9665 12.9665 13.75 12 13.75C11.0335 13.75 10.25 12.9665 10.25 12C10.25 11.0335 11.0335 10.25 12 10.25ZM20 10.25C20.9665 10.25 21.75 11.0335 21.75 12C21.75 12.9665 20.9665 13.75 20 13.75C19.0335 13.75 18.25 12.9665 18.25 12C18.25 11.0335 19.0335 10.25 20 10.25Z"
        fill={color}
      />
    </svg>
  );
};

ThreeDotsHorizontal24.category = 'Interface General';

export default ThreeDotsHorizontal24;
