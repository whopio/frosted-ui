import * as React from 'react';
import { IconProps } from './types';

export const DoubleChevronUpBold16 = ({ color = 'currentColor', ...props }: IconProps) => {
  return (
    <svg
      width="16"
      height="16"
      viewBox="0 0 16 16"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      data-fui-icon="DoubleChevronUpBold16"
      {...props}
    >
      <path
        d="M13.292 8.558c.39.39 1.023.39 1.414 0 .39-.39.39-1.024 0-1.414L8.963 1.4C8.43.869 7.568.869 7.036 1.4L1.293 7.144c-.39.39-.39 1.023 0 1.414.39.39 1.023.39 1.414 0l5.292-5.292 5.293 5.292zm0 6.148c.39.39 1.023.39 1.414 0 .39-.39.39-1.023 0-1.414L8.963 7.55c-.532-.532-1.395-.532-1.927 0l-5.743 5.743c-.39.39-.39 1.024 0 1.414.39.39 1.023.39 1.414 0l5.292-5.292 5.293 5.292z"
        fill={color}
        fillRule="evenodd"
        clipRule="evenodd"
      />
    </svg>
  );
};

DoubleChevronUpBold16.category = 'Arrows';

export default DoubleChevronUpBold16;
